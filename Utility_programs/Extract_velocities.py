...
files = list(QtWidgets.QFileDialog.getOpenFileName(None,\
...
# Read velocity file. Sign of Y coordinates is inverted since PyGimli uses positive
...
if mode == 0:
    ...
    results, okButton = dialog(["Velocity [m/s]"],["e"],[v_search],"extracted velocity")
    v_search = float(results[0])
elif mode == 1:
    x_extract = (x.max()+x.min())*0.5
    results, okButton = dialog(["Position of section [m]"],["e"],[x_extract],"extracted column")
    x_extract = float(results[0])
else:    
    y_extract = (y.max()+y.min())*0.5
    results, okButton = dialog(["Position of section [m]"],["e"],[y_extract],"extracted row")
